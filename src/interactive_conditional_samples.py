...
def interact_model(
    ...
    """
    # read .csv
    with open('data/GBcomments.csv', encoding='utf8') as f:
        db_comments = []
        for _ in range(1000):
            db_comments.append(f.readline().split(',')[1].replace('"', ''))
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        while True:
            # raw_text = input("Add comment >>> ")
            # while not raw_text:
            #     print('Prompt should not be empty!')
            #     raw_text = input("Add comment >>> ")
            comment = random.choice(db_comments)
            print('raw is', comment)
            raw_text = comments + f"1: {comment}\n2: "
            print("crafted is", raw_text)
